...
long long calcRangeLowestLocation(
    ...
    long long seedLowerBound,
    long long seedRange
){
    long long lowestLocation = std::numeric_limits<long long>::max();

    for(int i = seedLowerBound; i <= seedLowerBound + seedRange; i++){
        long long soilValue = getDestinationValue(i, seedToSoilMap);
    ...
    // cout << "Range lowest Location: " << lowestLocation << endl;
...
void run(const string filename) {
    ...
    long long overallLowestLocation = std::numeric_limits<long long>::max(); // Initialize to maximum possible int value;

    std::size_t length = seeds.size();

    for(int i = 0; i < length-1; i++){
        overallLowestLocation = min(
            overallLowestLocation,
            calcRangeLowestLocation(
                seedToSoilMap,
                soilToFertilizerMap,
    ...
    cout << "Overall lowest Location: " << overallLowestLocation << endl;
}
